...
public class SlotController : MonoBehaviour
{
    ...
    public IEnumerator Payout(int payout, int type = -1)
    {
        ...
        switch(type)
        {
            case 0:
                ...
                audioController.PlayWinLarge();
                audioController.SilencePayout(3f);
                break;
            case 1:
                gold = Instantiate(goldMedium, normalGoldSpawn.position, Quaternion.identity);
                Destroy(gold, 10f);

                audioController.PlayWinMedium();
                audioController.SilencePayout(2f);
                break;
            case 2:
                gold = Instantiate(goldSmall, normalGoldSpawn.position, Quaternion.identity);
                Destroy(gold, 10f);

                audioController.PlayWinSmall();
                audioController.SilencePayout(1f);
                break;
            case 3:
                gold = Instantiate(grandGold, grandGoldSpawn.position, Quaternion.identity);
                Destroy(gold, 10f);

                audioController.PlayWinGrand();
                audioController.SilencePayout(5f);
                break;
        }

        audioController.StartCounterSound();
        ...
        for (int i = 0; i < numOfSteps; i++)
        {
            ...
            audioController.SetCounterPitch(1 + 0.01f * i * 0.1f);
        ...
        audioController.StopCounterSound();
    }
        ...
        for(int i = 0; i < cannonballReels.Length; i++)
        {
            if (cannonballReels.Length == 3)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    audioController.PlayCannonballGet(1 + ((i * 3) + j) * 0.1f);
            ...
            else
            {
                ...
                audioController.PlayCannonballGet(1 + i * 0.1f);
    ...
    public void CalculateTheoreticalRTP()
    {
        ...
        int shipLife = pirateshipController.GetRandomLife();
        float hitChance = pirateshipController.GetRandomHitChance();
        ...
        for (int i = 0; i < 5000000; i++)
        {
            ...
            for (int j = 0; j < reelPositions.Length; j++)
            {
                ...
                if (reelPositions[j].ReelPositionType == ReelPositionType.Seven)
                {
                    cannonCount++;
                }
            }

            if (counts[(int)ReelPositionType.Seven] >= 3)
            {
                cannonCount += 6;
            }
            ...
            if (cannonCount > 0)
            {
                while (cannonCount > 0)
                {
                    ...
                    if (hit < hitChance)
                    {
                        ...
                        if (shipLife <= 0)
                        {
                            winningAmount += shipDestroyedPayout;

                            hitChance = pirateshipController.GetRandomHitChance();
                            shipLife = pirateshipController.GetRandomLife();
        ...
        print("T-RTP: " + (float)winningAmount / totalSpent);
        uiController.SetTRTP((float)winningAmount / totalSpent);
    }
